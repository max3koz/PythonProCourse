...
def generate_log_file(filename: str, lines: int = 100_000, error_rate: float = 0.01) -> None:
    """
    ...
    with open(filename, "w", encoding="utf-8") as f:
        for _ in range(lines):
            level = random.choices(LOG_LEVELS, weights=[1-error_rate, 0.05, error_rate, 0.1])[0]
            message = random.choice(MESSAGES)
            f.write(f"[{level}] {message}\n")
    print(f"Generated the '{filename}' file with {lines} lines.")

def generate_multiple_logs(folder: str, count: int = 3, lines_per_file: int = 100_000) -> List[str]:
    """
